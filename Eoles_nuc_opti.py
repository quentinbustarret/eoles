...
# Result : Current version, works great for 1 year

# Forth implementation : using a LP model
# Result : Developpement in progress
...
elif nb_years == 2 or nb_years == 20:
...
def select_mins(aggregated):
    ...
    for test_min in range(step): # We test every value for the first outage, using the minimum for the first year does not imply that the next ones will minimize the sum
        ...
        if impossible_values.at[test_min,'value'] == 0: # For Lynch constraints
            continue
        if nb_years == 20 and test_aggregated.at[test_min,"aggregated_deficit_("+str(len_outage/24)+"d)"] > 0:
            continue
...
def step_by_step_plot(ini_aggregate, aggregate, mins, i): 
    # The evolution of the production deficit is not plotted because it is not very exploitable
    bx=aggregate.plot(x='hour', y='aggregated_deficit_('+str(len_outage/24)+'d)', kind='line')
    ini_aggregate.plot(ax=bx, x='hour', y='aggregated_deficit_('+str(len_outage/24)+'d)', linestyle='--')
    plt.xlabel("Hours")
    plt.ylabel("Deficit (GWh)")
    for min in mins:
        plt.axvline(x=min)
    plt.title(str(i) + " outage(s) scheduled")
    plt.show()
    plt.clf()
    plt.close()
...
def main():
    ...
    for i in range(nb_plant):
        ...
        if Step_by_step_plots and With_plots: step_by_step_plot(ini_aggregate, aggregated, mins, i)
        for min in mins:
    ...
    if With_plots:
        ax=operating_capacity.plot(x='hour', y='capa', kind='line')
        operating_minimum.plot(ax=ax, x='hour', y='min', kind='line')
        plt.xlabel("Hours")
        plt.ylabel("Operating capacity (GW)")
        plt.title(str(nb_plant)+" outages scheduled")
        #plt.savefig("operating_capacity_"+str(nb_plant)+"PP_1months.png")
        bx=aggregated.plot(x='hour', y='aggregated_deficit_('+str(len_outage/24)+'d)', kind='line')
        ini_aggregate.plot(ax=bx, x='hour', y='aggregated_deficit_('+str(len_outage/24)+'d)', linestyle='--')
        plt.xlabel("Hours")
        plt.ylabel("Aggregated deficit (GWh)")
        plt.title(str(nb_plant)+" outages scheduled")
        #plt.savefig("aggregated_deficit_3month.png")
        plt.show()
...
in_fix_capa = pd.read_csv("inputs/fix_capas.csv", header=None)
...
out_fix_capa.to_csv(path_or_buf='./inputs/fix_capas.csv', index=False, header= False)

operating_capacity.to_csv(path_or_buf='./inputs/maximal_operating_capacity_'+str(nb_years)+'y.csv', header= False, index=False)

operating_minimum.to_csv(path_or_buf='./inputs/minimal_operating_capacity_'+str(nb_years)+'y.csv', header= False, index=False)
