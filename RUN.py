...
# For standard use, you may change all inputs files and the model_name below
...
while(True):
    in_fix_capa = pd.read_csv("inputs/fix_capas.csv", index_col=0, header=None).squeeze("columns")
    try:
        in_fix_capa.drop("nuc", inplace=True)
        in_fix_capa.to_csv(path_or_buf='./inputs/fix_capas.csv', header= False)
